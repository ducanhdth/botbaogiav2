// api/generate-docx.js — Vercel Serverless: generates .docx from quote data
import {
...
function numToWords(n) {
  ...
  function tri(x){if(!x)return"";const h=Math.floor(x/100),t=Math.floor((x%100)/10),o=x%10;let s="";if(h)s+=u[h]+" trăm ";if(t===0&&o&&h)s+="linh ";if(t===1){s+="mười ";if(o===5)s+="lăm ";else if(o)s+=u[o]+" ";}else if(t>1){s+=u[t]+" mươi ";if(o===1)s+="mốt ";else if(o===5)s+="lăm ";else if(o)s+=u[o]+" ";}else if(o)s+=u[o]+" ";return s.trim();}
  const ty=Math.floor(n/1e9),tr=Math.floor((n%1e9)/1e6),ng=Math.floor((n%1e6)/1e3),du=n%1e3;
  let r="";if(ty)r+=tri(ty)+" tỷ ";if(tr)r+=tri(tr)+" triệu ";if(ng)r+=tri(ng)+" nghìn ";if(du)r+=tri(du);
...
function tc(children,w,opts={}){return new TableCell({borders:AB,margins:CM,width:{size:w,type:WidthType.DXA},children:Array.isArray(children)?children:[children],...opts});}

async function buildDoc(quote, co) {
  ...
  const CW6=CW.slice(0,6).reduce((a,b)=>a+b,0);
  ...
  const doc = new Document({
    ...
    sections:[{
      ...
      children:[
        // HEADER
        new Table({width:{size:PW,type:WidthType.DXA},columnWidths:[Math.round(PW*.73),Math.round(PW*.27)],
          borders:{top:NBD,bottom:{...BD,size:12},left:NBD,right:NBD,insideH:NBD,insideV:NBD},
          rows:[new TableRow({children:[
            new TableCell({width:{size:Math.round(PW*.73),type:WidthType.DXA},borders:NB,margins:{top:0,bottom:120,left:0,right:0},children:[
              p([rb(coN,{size:26})]),
            ...
            new TableCell({width:{size:Math.round(PW*.27),type:WidthType.DXA},borders:NB,verticalAlign:VerticalAlign.CENTER,children:[
        ...
        ep(200),
        p([rb("BẢNG BÁO GIÁ SẢN PHẨM & DỊCH VỤ",{size:34})],{alignment:AlignmentType.CENTER,spacing:{before:100,after:200}}),
        // CUSTOMER
        new Table({width:{size:PW,type:WidthType.DXA},columnWidths:[Math.round(PW*.55),Math.round(PW*.45)],
          borders:{top:NBD,bottom:NBD,left:NBD,right:NBD,insideH:NBD,insideV:NBD},
          rows:[new TableRow({children:[
            new TableCell({width:{size:Math.round(PW*.55),type:WidthType.DXA},borders:NB,children:[
            ...
            new TableCell({width:{size:Math.round(PW*.45),type:WidthType.DXA},borders:NB,children:[
        ...
        new Table({width:{size:PW,type:WidthType.DXA},columnWidths:CW,rows:[
          new TableRow({tableHeader:true,children:
            [["STT",null],["Tên SP & Quy cách kỹ thuật",AlignmentType.LEFT],["Hình ảnh\n(Mockup)",null],["ĐVT",null],["Số\nLượng",null],["Đơn giá\n(VNĐ)",null],["Thành tiền\n(VNĐ)",null]]
            .map(([h,al],ci)=>new TableCell({borders:AB,margins:CM,width:{size:CW[ci],type:WidthType.DXA},shading:{fill:"D9D9D9",type:ShadingType.CLEAR},verticalAlign:VerticalAlign.CENTER,children:[p([rb(h)],{alignment:al||AlignmentType.CENTER})]}))
          }),
          ...
          new TableRow({children:[new TableCell({borders:AB,margins:CM,columnSpan:6,width:{size:CW6,type:WidthType.DXA},children:[p([rb("Cộng tiền hàng (Subtotal):")],{alignment:AlignmentType.RIGHT})]}),tc([p([rb(fmtN(sub))],{alignment:AlignmentType.RIGHT})],CW[6])]}),
          new TableRow({children:[new TableCell({borders:AB,margins:CM,columnSpan:6,width:{size:CW6,type:WidthType.DXA},children:[p([r("Thuế GTGT / VAT ("+vr+"%):")],{alignment:AlignmentType.RIGHT})]}),tc([p([r(fmtN(va))],{alignment:AlignmentType.RIGHT})],CW[6])]}),
          new TableRow({children:[new TableCell({borders:AB,margins:CM,columnSpan:6,width:{size:CW6,type:WidthType.DXA},children:[p([rb("TỔNG CỘNG TIỀN THANH TOÁN (GRAND TOTAL):")],{alignment:AlignmentType.RIGHT})]}),tc([p([rb(fmtN(tot))],{alignment:AlignmentType.RIGHT})],CW[6])]}),
          new TableRow({children:[new TableCell({borders:AB,margins:CM,columnSpan:7,width:{size:PW,type:WidthType.DXA},children:[p([r("(Bằng chữ: "),rb(numToWords(tot)),r(")")])]})]})
        ]}),
        // TERMS
        ep(220),
        ...
        new Table({width:{size:PW,type:WidthType.DXA},columnWidths:[Math.round(PW*.55),Math.round(PW*.45)],
          borders:{top:NBD,bottom:NBD,left:NBD,right:NBD,insideH:NBD,insideV:NBD},
          rows:[new TableRow({children:[
            new TableCell({width:{size:Math.round(PW*.55),type:WidthType.DXA},borders:NB,children:[ep(0)]}),
            new TableCell({width:{size:Math.round(PW*.45),type:WidthType.DXA},borders:NB,children:[
